...
!	1.
real (kind = 8) structparams(nparams_pc1), spsimplex(nparams_pc1+1,nparams_pc1) !STRUCTURAL PARAMETERS OF THE MODEL, THE ONES TO BE CALIBRATED; and corresponding simplex for Nelder-Mead algorithm
!	The parameters are:	ltot kappa d cmin beta delta eta sigma cgamma lgamma xi nu
real (kind = 8) y(nparams_pc1), funevals(nparams_pc1+1)
...
structparams = (/4.0d3, 9.0d2, 5.0d4, 2.6d3, 0.95d0, 0.2d0,  6.8d0, 1.75d0, 0.5d0, 0.5d0, 1.4d0, 2.0d0/)

!parameter values:   4133.8722262683023        969.45245518765523        96280.840349740465        2590.7473340076281        1.1215418028954651       0.20456010196180929        1.7160613755137539        1.6901410039156692       0.51989746103720047       0.47101004770116761        1.3978070526029853        1.7194029525627124
...
!I put no weight on assets moment for now, since the differences are huge.
	...
	print *, 'Total number of simplex vectors:', nparams_pc1+1
...
contains
	...
	!	Function takes a vector of structural parameters ("strpar") and spits out norm of moment conditions as a function result.
		...
		use nrtype
		implicit none
		real (DP), dimension(:), intent(in)	:: strpar ! ltot kappa d cmin beta delta eta sigma cgamma lgamma xi nu
...
!		Local var
	...
	!	2. Calculate value function given these parameters
		...
		open(unit=11,file='data_output/life_assets_pcost1.txt',status='replace')
		open(unit=12,file='data_output/life_labor_pcost1.txt',status='replace')
		open(unit=13,file='data_output/life_earnings_pcost1.txt',status='replace')
		open(unit=14,file='data_output/life_wage_pcost1.txt',status='replace')
		open(unit=15,file='data_output/life_medexp_pcost1.txt',status='replace')
		open(unit=16,file='data_output/app_age_pcost1.txt',status='replace')
...
integer, dimension(lifespan) :: wl, ml, hl
integer death_age
