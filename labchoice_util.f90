subroutine labchoice (lchoice, cons, util, acur,anext, h, wage, mexp,ss,age,cohort)
    ...
    real (kind = 8) :: ltot_scaled, phi_scaled
			...
			if (lbr == 0) then !labor is zero
				if (anxt == 0) then !next-period assets are zero: agent is eligible for minimal consulption level					
					if (cns > cmin_scaled) then !consumption is higher than minimal, no transfers
						utl = (cns**ugamma*(ltot_scaled-(2-h)*phi_scaled)**(1-ugamma))**(1-sigma)/(1-sigma) !NO KAPPA IN UTILITY: zero labor
					else
				...
				if (anxt == 0) then !next-period assets are zero: agent is eligible for minimal consumption level					
					if (cns > cmin_scaled) then
						if (ltot_scaled-lbr-kappa_scaled-xi_scaled*(age-age0)**nu-(2-h)*phi_scaled >=0) then 
							utl = (cns**ugamma*(ltot_scaled-lbr-kappa_scaled-xi_scaled*(age-age0)**nu-(2-h)*phi_scaled) &
