...
return !end of subroutine
    ...
    contains
        ...
        subroutine cons_util_calc(lbr, wg, acrnt, anxt, mxp, ssec, cns, utl)
			...
			income = r(cohort)*acrnt + earnings !asset return + labor income
			...
			cns = (1+r(cohort))*acrnt + lbr*wg +ssec_received- anxt - mxp - taxes !uncompensated consumption; in the case when lbr==0, labor income is ZERO
			if (lbr == 0) then !labor is zero
				if (anxt == 0) then !next-period assets are zero: agent is eligible for minimal consulption level					
					if (cns > cmin_scaled) then !consumption is higher than minimal, no transfers
						utl = ((1+delta*(h-1))*(cns**cgamma*ltot_scaled**lgamma)**(1-sigma))/(1-sigma) !NO KAPPA IN UTILITY: zero labor
					else
				...
				if (anxt == 0) then !next-period assets are zero: agent is eligible for minimal consumption level					
					if (cns > cmin_scaled) then
						utl = ((1+delta*(h-1))*(cns**cgamma*(ltot_scaled-lbr-kappa_scaled-xi_scaled*(age-age0)**nu) &
						**lgamma)**(1-sigma))/(1-sigma) 
					else
