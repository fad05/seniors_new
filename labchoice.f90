subroutine labchoice (lchoice, cons, util, acur,anext, h, wage, mexp)
    ...
    !SCALED VARIABLES
    real (kind = 8) acur_scaled, anext_scaled,mexp_scaled
...
!	Recall: c = (1+r)*a +w*l - a' - m
!	Currently c,a, m are expressed in US dollars, and l in hours, and w in 1usd/1hr.
!	We would like to express (C, A, A', M) in 1000s of US dollars, (W) in 1000USD/1000hrs and L in 1000s of hours.
!	C = c/1000 (thd USD); A = a/1000 (thd USD); M = m/1000;
!	L = l/1000 (1000hrs).
!	NOTE: we don't need to scale wage!
!	W = w (since 1000USD/1000hrs = 1USD/1hr). 
!	Then:
!	c/1000 = C = (1+r)*A + WL - A' - M = (1+r)a/1000 + w*l/1000 -a'/1000 -m/1000
...
!=========================END SCALING========================================================================================
    ...
    do i = 1,100 !on this labor grid, find corresponding utility and consumption
	    call cons_util_calc(labgrid(i), wage, acur_scaled, anext_scaled, mexp_scaled, cgrid(i), ugrid(i))
    enddo
    ...
    call cons_util_calc(0.0d0,wage,acur_scaled,anext_scaled,mexp_scaled,c0,u0)
    ! Utility and cons at first grdpoint (1 hour of work)
    call cons_util_calc(labgrid(1),wage,acur_scaled, anext_scaled,mexp_scaled,ctemp,utemp)
...
!	We're looking for maximum 
		...
		do while (c-a>1.0d-3) !the tolerance is ohe hour (1E-3 of 1000s of hours)
			if (b-a<=c-b) then
				dnew = (b+c)/2.0
				call cons_util_calc(dnew, wage, acur_scaled, anext_scaled, mexp_scaled, cd, ud)  
			else
				dnew = (a+b)/2.0
				call cons_util_calc(dnew, wage, acur_scaled, anext_scaled, mexp_scaled, cd, ud)  
			endif
    ...
    contains
        subroutine lc_minpack( m, n, x, fvec1, iflag )
			...
			if (lab > lmax) then !choice is infeasible
				...
				call cons_util_calc(lab, wage, acur_scaled, anext_scaled, mexp_scaled, cl, ul)
